...
- 2026-07-12: eval --live baseline attempt: INVALID (one-shot hermes-agent harness is OpenRouter-wired; 404 with no OPENROUTER_API_KEY; Anthropic compat base_url also 404). Verdict: static gate (check_skills.py, 6/6) = the pre-deploy tool; live evals via each case's documented manual Telegram path, or add an OpenRouter key later. Baseline table from the 404 run discarded.
- 2026-07-12: langfuse_ops.py harvested + verified live (list works against real traces; smoke OK). L5 observability demo: list → diff --a --b → alert --max-cost 0.01 (fires) / --max-cost 100 (silent, exit 0). Alert cron documented in narada/README.
- 2026-07-12: eval --live REAL baseline: 15/15 checks, 6/6 agents PASS. Runner rewired to direct Anthropic Messages API (SKILL.md as system prompt = production parity, claude-haiku-4-5, no OpenRouter). Supersedes the discarded 404 run. Evals parameter: L4 evidence = static gate + live baseline + version-labeled outputs in git.
